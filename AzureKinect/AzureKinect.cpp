...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    time_t t = time(0);
    char today[32] = { NULL };
    strftime(today, sizeof(today), "%m_%d_%H_%M_%S", localtime(&t));
    std::string folder{ today };
    std::filesystem::create_directory(folder);
    std::string pathRGB{ "./" + folder + "/rgb" }, pathDepth{ "./" + folder + "/depth" }, pathIr{ "./" + folder + "/ir" };
    ...
    rgb_out.open(folder + "/0-rgb.txt");
    d_out.open(folder + "/0-depth.txt");
    ir_out.open(folder + "/0-ir.txt");
    ...
    //for (size_t i = 0; i < 1; i++)
    {
        ...
        if (device.get_capture(&capture)) {
            ...
            cv::imwrite(pathRGB + filename_rgb, cv_rgbImage_no_alpha);
            cv::imwrite(pathDepth + filename_d, cv_depth);
            //cv::imwrite(pathIr + filename_ir, cv_irImage);
